...
from cobra4.tools.bench import (
    BenchResult, run_benchmarks, time_budget,
    format_table, format_compare,
)
...
def test_run_benchmarks_with_all_targets_returns_one_result_each() -> None:
    ...
    assert {"parser", "codegen", "smart-dispatch", "workflow", "async-parallel"} <= names
...
def test_cli_bench_writes_json(tmp_path: Path) -> None:
    ...
    code, _ = _run_c4(["bench", "parser", "--seconds", "0.05",
                       "--json", str(out_path)], tmp_path)
    assert code == 0
...
def test_cli_bench_compare_against_baseline(tmp_path: Path) -> None:
    ...
    code, out = _run_c4(["bench", "parser", "--seconds", "0.05",
                         "--compare", str(baseline)], tmp_path)
    assert code == 0, out
    assert "Δ%" in out
...
def test_cli_bench_subset_filters_targets(tmp_path: Path) -> None:
    ...
    _run_c4(["bench", "parser", "codegen", "--seconds", "0.05",
             "--json", str(out_path)], tmp_path)
    data = json.loads(out_path.read_text())
